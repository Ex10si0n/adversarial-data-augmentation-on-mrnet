...
def plot_combiner(p1=None, p2=None, p3=None, pattern="", labels=[]):
    ...
    for i in range(len(train)):
        # save fs
        train_label = metrics[train[i]]
        train_label = train_label.replace("_", "-")
        val_label = metrics[val[i]]
        val_label = val_label.replace("_", "-")
        label = train_label.split("-")[1]
        ...
        plt.plot(p1[:, 0], p1[:, train[i]+1], label='train ' + labels[0])
        plt.plot(p1[:, 0], p1[:, val[i]+1], label='val ' + labels[0])
        if p2 is not None:
            plt.plot(p2[:, 0] + 50, p2[:, train[i]+1], label='train ' + labels[1])
            plt.plot(p2[:, 0] + 50, p2[:, val[i]+1], label='val ' + labels[1])
        if p3 is not None:
            plt.plot(p3[:, 0] + 50, p3[:, train[i]+1], label='train ' + labels[2])
            plt.plot(p3[:, 0] + 50, p3[:, val[i]+1], label='val ' + labels[2])
        plt.legend()
...
def plot_learning_curve(a1, a2, b, t, d, pattern):
    info = ''
    '''
    b: data for baseline
    a1: data for adv_1
...
for t in task:
    for d in dim:
        ...
        for i in range(1, 8):
            plot_learning_curve(adv_1, adv_2, baseline, t, d, i)
