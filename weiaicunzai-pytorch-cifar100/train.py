...
def adversarial_train(epoch, checkpoint=checkpoint, epsilon=epsilon, percent=percent):
    start = time.time()
    samples = 0
    net.train()
    ...
    for batch_index, (images, labels) in enumerate(cifar100_training_loader):
        samples += 1
        if samples >= percent * len(cifar100_training_loader):
            break
        ...
        optimizer.zero_grad()
        perturbed_images = fgsm_attack(net, images, labels, epsilon, device='cuda')
